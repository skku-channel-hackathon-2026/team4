...
> 팀 레포는 **PR 머지 → main CI 성공 → 원격 D1 마이그레이션 → Vercel 자동 배포** 순서입니다. 빌드·실행 대기 시간이 필요합니다.
...
서버는 Vercel Hobby, DB는 기존 팀 전용 D1입니다. 운영 배포 시스템이 자동으로 연결합니다.
...
채널톡에서 `/망선박`으로 연 학생 대화와 개발 환경에서 `음성 시연`을 펼쳐
