 ...
 * 성균관대학교 학부 전공 목록.
 *
 * 화면에서는 단과대학을 고른 뒤 학과를 고르는 2단계 선택으로 쓴다.
 *
 * 지금은 상황에 기록해 확인·비교 화면에 표시하기만 한다. 검색 순위에는 아직
 * 쓰지 않는다. 학과별 학사 규정 차이를 사례 조건에 반영하는 것이(v2 §13) 다음
 * 단계이고, 그때 `retrieval.service`가 이 값을 읽게 된다. 화면 문구도 지금
 * 하는 일까지만 약속한다.
 *
