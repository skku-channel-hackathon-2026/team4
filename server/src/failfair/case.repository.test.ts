...
test("옛 app_records 배열에 있던 사례는 첫 읽기 때 한 번만 표로 옮긴다", async () => {
  await inDatabase(async (repository) => {
    await setRecord("failfair:cases", [
      realCase({ id: "legacy-1", status: "approved" }),
      { broken: true },
      realCase({ id: "legacy-2" }),
    ]);
    const all = await repository.list();
    assert.ok(all.some((item) => item.id === "legacy-1"));
    assert.ok(all.some((item) => item.id === "legacy-2"));
    assert.deepEqual(
      (await getRecord<{ count: number }>("failfair:cases:migrated"))?.count,
      2,
    );

    // 표식이 남은 뒤에는 옛 배열이 바뀌어도 다시 옮기지 않고, 새 저장소의 변경을 덮지 않는다.
    await repository.save(
      realCase({ id: "legacy-2", title: "새 저장소에서 고침" }),
    );
    await setRecord("failfair:cases", [realCase({ id: "legacy-3" })]);
    const again = new D1CaseRepository(DEMO_CASES);
    assert.ok(!(await again.list()).some((item) => item.id === "legacy-3"));
    assert.equal((await again.get("legacy-2"))?.title, "새 저장소에서 고침");
  });
});
